...
The objects are small (ten 730×730 float32 matrices ≈ 20 MB) and depend only
on the row mask, the kept-column set, and the column-group labels — not on
the response. A hit skips the 23–43 s convert+GEMM pass.
"""
...
def loco_cache_digest(
    ...
    *,
    ...
    hasher.update(b"loco-v1\0")
    hasher.update(np.int64(n_markers).tobytes())
    hasher.update(np.int64(max_line).tobytes())
...
def save_loco_kinship(path: Union[str, Path], loco: LocoKinship) -> Path:
    ...
    arrays = {
        ...
        "total_raw": np.ascontiguousarray(loco._total_raw),
        "total_diag": np.ascontiguousarray(loco._total_diag),
    }
